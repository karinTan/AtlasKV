"""Copied Android World T3A/M3A prompt construction utilities."""
...
PROMPT_PREFIX = T3A_PROMPT_PREFIX
...
def generate_t3a_ui_elements_description_list_full(
    ui_elements: list[representation_utils.UIElement],
    screen_width_height_px: tuple[int, int],
) -> str:
  """Copied from `android_world.agents.t3a._generate_ui_elements_description_list_full`."""
  tree_info = ''
  for index, ui_element in enumerate(ui_elements):
    if representation_utils.validate_ui_element(
        ui_element, screen_width_height_px
    ):
      tree_info += f'UI element {index}: {str(ui_element)}\n'
  return tree_info


def _generate_m3a_ui_element_description(
    ui_element: representation_utils.UIElement, index: int
) -> str:
  """Copied from `android_world.agents.m3a._generate_ui_element_description`."""
...
def generate_m3a_ui_elements_description_list(
    ui_elements: list[representation_utils.UIElement],
    screen_width_height_px: tuple[int, int],
) -> str:
  """Copied from `android_world.agents.m3a._generate_ui_elements_description_list`."""
  tree_info = ''
  for index, ui_element in enumerate(ui_elements):
    if representation_utils.validate_ui_element(
        ui_element, screen_width_height_px
    ):
      tree_info += _generate_m3a_ui_element_description(ui_element, index) + '\n'
  return tree_info
...
def action_selection_prompt(
  ...
  """Builds the Android World action-selection prompt."""
  if history:
    history_text = '\n'.join(history)
  else:
  ...
  if agent == 'm3a':
    return (
        ...
        + ' (notice that some elements in this list may not be visible in the'
        + ' current screen and so you can not interact with it, can try to'
        + ' scroll the screen to reveal it first), the numeric indexes are'
...
def action_to_history_summary(action: dict, instruction: str | None = None) -> str:
  """Fallback summary for offline data when teacher summaries are unavailable."""
  summary = (
      'Action selected: '
